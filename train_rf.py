...
def main():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "rf_parity.png"), dpi=300)
    plt.close()
    
    # Save metrics txt
    with open(os.path.join(out_dir, "rf_metrics.txt"), 'w', encoding='utf-8') as f:
    ...
    pdp_features = ['invT_1000_per_K', 'cyclic_or_acyclic']
    importances = pipe.named_steps['model'].feature_importances_
    df_rf_imp = pd.DataFrame({'Feature': feature_cols, 'Importance': importances}).sort_values('Importance', ascending=False)
    top_descriptors = [col for col in df_rf_imp['Feature'] if col not in ['invT_1000_per_K', 'cyclic_or_acyclic']][:2]
    pdp_features.extend(top_descriptors)
    
    PartialDependenceDisplay.from_estimator(
        pipe, X, features=pdp_features, ax=axes.ravel()[:len(pdp_features)]
    )
    plt.suptitle("Partial Dependence Plots for Optimized Random Forest Model", fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "rf_pdp.png"), dpi=300)
    plt.close()
    ...
    try:
        ...
        plt.title("SHAP Beeswarm Plot (Optimized Random Forest)", fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, "rf_shap.png"), dpi=300)
        plt.close()
