...
def main():
    ...
    fold_metrics = []
    print("\nTraining FNN (MLPRegressor) with GroupKFold...")
    ...
    plt.scatter(all_true, all_pred, alpha=0.6, color='#9467bd', edgecolors='k')
    ...
    plt.title("Neural Network (FNN) Parity Plot")
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fnn_parity.png"), dpi=300)
    plt.close()
    
    # Save metrics txt
    with open(os.path.join(out_dir, "fnn_metrics.txt"), 'w', encoding='utf-8') as f:
        f.write("Model: Neural Network (FNN)\n\n")
    ...
    # Save Loss Curve
    mlp_model = pipe.named_steps['model']
    plt.figure(figsize=(6, 4))
    plt.plot(mlp_model.loss_curve_, color='#34495e', lw=2)
    plt.xlabel("Epoch")
    plt.ylabel("Loss (MSE)")
    plt.title("FNN Loss Curve")
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fnn_loss.png"), dpi=300)
    plt.close()
    print("Saved FNN loss curve.")
